...
import { appendSignalLog } from './SignalLogPanel.jsx';
...
export default function StockCard({ ticker, isWatchlisted, onToggleWatchlist }) {
  const { loading, error, data, loadIntraday } = useStockData(ticker);
  const [mode, setMode] = useState('swing');

  const baseJudgement = useMemo(() => {
    if (!data) return null;
  ...
  const preliminarySignal = useMemo(
    () => (data && baseJudgement ? buildTradeSignal(data, baseJudgement, rule, targets) : null),
    [data, baseJudgement, rule, targets]
  );
  const judgement = useMemo(() => {
    if (!baseJudgement || !preliminarySignal) return baseJudgement;
    const entryStage =
      preliminarySignal.action === 'READY'
        ? 'ENTRY'
        : preliminarySignal.action === 'WAIT'
        ? 'WATCH_ONLY'
        : 'REJECT';
    const strategy =
      entryStage === 'ENTRY' ? 'BUY' : entryStage === 'WATCH_ONLY' ? 'WATCH_ONLY' : 'REJECT';
    const safetyReason =
      preliminarySignal.blockers[0] ?? preliminarySignal.cautions[0] ?? '안전 필터 확인 필요';
    const reason =
      entryStage === baseJudgement.entryStage
        ? baseJudgement.reason
        : `${baseJudgement.reason} · 통합 안전필터: ${safetyReason}`;

    return { ...baseJudgement, entryStage, strategy, reason };
  }, [baseJudgement, preliminarySignal]);
  const summary = useMemo(
    () => (data && judgement ? buildSummary(data, judgement, rule) : null),
    [data, judgement, rule]
  );
  const tradeSignal = preliminarySignal;
  const chartReferenceLines = useMemo(() => {
    if (!data) return null;
    const highlightedTarget = targets.find((t) => t.label === rule.highlightHorizon);
    return {
      prevClose: data.price - data.change,
      high52w: data.technicals?.fiftyTwoWeek?.high ?? null,
      low52w: data.technicals?.fiftyTwoWeek?.low ?? null,
      target: highlightedTarget?.target ?? null,
      stop: highlightedTarget?.stop ?? null,
    };
  }, [data, targets, rule.highlightHorizon]);

  useEffect(() => {
    if (!tradeSignal || !judgement) return;
    appendSignalLog({
      ticker,
      mode,
      modeLabel: rule.label,
      action: tradeSignal.action,
      grade: judgement.finalGrade,
      stage: judgement.entryStage,
    });
  }, [ticker, mode, rule.label, tradeSignal, judgement]);
        ...
        <button
          ...
          className={`grid h-9 w-9 shrink-0 place-items-center rounded-lg border transition ${
            isWatchlisted
              ...
              : 'border-gray-100 bg-white text-gray-300 hover:border-amber-200 hover:text-amber-500'
          }`}
          aria-label={isWatchlisted ? '관심종목에서 제거' : '관심종목에 추가'}
          title={isWatchlisted ? '관심종목에서 제거' : '관심종목에 추가'}
        >
          <Star size={17} className={isWatchlisted ? 'fill-current' : ''} />
        </button>
      </header>
          ...
          <div className="border-t border-gray-100 pt-4 -mx-1">
            ...
            {judgement ? (
              <div className="bg-blue-50/40 border border-blue-100/60 rounded-xl p-3 flex flex-col gap-3">
                ...
                <TradeSignalPanel signal={tradeSignal} currency={data.currency} />
              </div>
            ) : null}
          </div>
        </>
